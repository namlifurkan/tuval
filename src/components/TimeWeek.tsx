    ...
    <div>
      <div className="mb-2 flex items-center gap-1">
        <button
          ...
          className="grid h-6 w-6 place-items-center rounded-md text-[#8A867C] hover:bg-[#EAE6DD]"
        >
          <ChevronLeft size={14} />
        </button>
        <button
        ...
        <span className="ml-1 text-[12px] font-semibold text-[#141310]">
      ...
      {!rows.length ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-[#E2DED5] bg-[#FCFBF8]">
          <table className="w-full border-collapse text-[12px]">
              ...
              <tr>
                ...
                {days.map((day) => (
                  <th
                    key={day}
                    scope="col"
                    className={`border-b border-[#E2DED5] px-2 py-2 text-right font-bold uppercase tracking-[0.1em]
                      ${day === today() ? 'text-[#C8452D]' : 'text-[#8A867C]'}`}
                  >{label(day)}</th>
                ))}
                <th scope="col" className="border-b border-[#E2DED5] px-3 py-2 text-right font-bold uppercase tracking-[0.1em] text-[#141310]">
                  {t('Total')}
                </th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.user} className="border-b border-[#EAE6DD] last:border-0">
                  <th scope="row" className="px-3 py-1.5 text-left font-normal text-[#141310]">
                    {nameOf(row.user)}
                  </th>
                  {row.days.map((minutes, i) => (
                    <td key={days[i]} className="px-2 py-1.5 text-right text-[#4A463E]">
                      {minutes ? readable(minutes) : <span className="text-[#D8D5CD]">·</span>}
                    </td>
                  ))}
                  <td className="px-3 py-1.5 text-right font-semibold text-[#141310]">
                    {readable(row.total)}
                  </td>
                </tr>
              ))}
