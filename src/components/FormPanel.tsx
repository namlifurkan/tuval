...
const field = 'mb-1 w-full rounded-md border border-hairline bg-paper px-2 py-1 text-[13px] outline-none focus:border-pigment'
...
export function FormPanel({ db, fields }: { db: Row; fields: Field[] }) {
  ...
  return (
    <Popover
      ...
      trigger={({ toggle }) => (
        <button
          type="button"
          onClick={toggle}
          className={`flex items-center gap-1.5 rounded-md px-2 py-1 text-xs font-semibold hover:bg-shade
            ${form?.active ? 'text-pigment' : 'text-muted hover:text-ink'}`}
        >
    ...
    >
      {() => (!form ? (
        <>
          <p className="mb-1.5 px-1 text-[11px] leading-snug text-muted">
        ...
        <>
          ...
          <p className="mb-1 mt-2 px-1 text-[11px] font-bold uppercase tracking-[0.12em] text-muted">
            {t('Asks for')}
          </p>
