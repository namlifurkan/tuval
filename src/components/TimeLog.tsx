...
export function TimeLog({ record, team }: { record: string; team: Teammate[] }) {
  ...
  return (
    <div className="mt-4">
      <span className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.13em] text-muted">
        <Timer size={11} /> {t('Time')}
        {!!total && <span className="text-ink">{readable(total)}</span>}
      </span>
      ...
      <div className="mt-1 flex gap-1">
        <input
          ...
          className="w-[86px] shrink-0 rounded-md border border-hairline bg-surface px-2 py-1 text-[12px] outline-none focus:border-pigment"
        />
        <input
          value={note}
          ...
          className="min-w-0 flex-1 rounded-md border border-hairline bg-surface px-2 py-1 text-[12px] outline-none focus:border-pigment"
        />
      </div>

      {held.map((stint) => (
        <div key={stint.id} className="group/stint mt-1 flex items-center gap-2 rounded-md px-1 py-0.5 text-[12px] hover:bg-tint">
          <span className="w-[52px] shrink-0 font-mono text-[11px] text-ink">
